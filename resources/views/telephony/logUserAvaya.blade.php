...
@section('content_header')
    <h1> Formulario de logs de login usuarios Avayas</h1><hr>
@stop
    ...
    @can('telephony-run')
        ...
        <div class="card card-default">
            <div class="card-header with-border" style="background: #dfe1e4;">
                <h3 class="card-title">Dattos de la búsqueda</h3>
            </div>
            <div class="card-body">
                            ...
                            <div>
                                <select class="form-control" name="customer" id="customer" required>
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->loginname }}">{{ $customer->loginname }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </td>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            send.addEventListener("click", function(){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('telephony.logUserAvaya') }}',
                    dataType: 'json',
                    data: {
                        'customer': customer,
                        'f_ini': f_ini,
