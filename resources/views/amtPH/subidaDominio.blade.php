...
@section('content_header')
    <h1> Formulario de ejecución de subida de dominio</h1><hr>
@stop
...
@section('content')
    @can('amtPH-run')
        ...
        <form action="{{ route('amtPH.subidaDominioStore') }}" method="post" id="formfield">
            {!! csrf_field() !!}
            <div class="card card-default">
                <div class="card-header with-border">
                    <h3 class="card-title">Formulario de ejecución de subida de dominio</h3>
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-hover">
                            ...
                            <td>
                                <div class="input-group">
                                    <select name="dominio" id="dominio" class="form-control" style="width: 100%" required>
                                        <option></option>
                                        <option value="co_datoscomp_wls_pr">co_datoscomp_wls_pr</option>
                                        <option value="CO_CTRAFICO_WLS_PRD">CO_CTRAFICO_WLS_PRD</option>
                                    </select>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <label for="cluster">{{ __('Cluster') }}</label>
                            </td>
                            <td>
                                <div class="input-group">
                                    <select name="cluster" id="cluster" class="form-control" style="width: 100%" required>
                                        <option></option>
                                        <option value="CLUSTER_DATOSCOMP">CLUSTER_DATOSCOMP</option>
                                    </select>
                                </div>
                            </td>
                        </tr>
