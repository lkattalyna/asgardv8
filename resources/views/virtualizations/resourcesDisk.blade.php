...
<h1>Formulario aumento de disco duro</h1><hr>
@stop
...
@section('content')
    ...
    @include('layouts.messages')
        <form action="{{ route('virtualization.resourcesDiskStore') }}" method="POST" id="formfield">
            {{ csrf_field() }}
            <div class="card card-default">
                <div class="card-header with-border" style="background: #dfe1e4;">
                    <h3 class="card-title">Datos para aumento de disco duro</h3>
                </div>
                <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on("click",".Vlink",function(e){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('virtualization.resourcesDisk') }}',
                    data: parametros2,
                    ...
                    success: function({data}) {
                        ...
                        //let length =  data.length; alert("pba"+length); return false;
                        const {disco,capacityDisk,datastore,AvailableSpace} = data;                     
                        ...
                        for(let networkadapterAux of disco){                          
                            ...
                            $(`#C_${networkadapterAux}`).append(document.querySelector(`#C_${networkadapterAux}`).value = AvailableSpace[i]);
                            $(`#CI_${networkadapterAux}`).val(AvailableSpace[i]);
                            i++;
                        }
                    }
                    });
