...
@section('content_header')
    <h1>Formulario para revertir snapshots</h1><hr>
@stop
...
@section('content')
    @can('virtualization-run')
        <form action="{{ route('virtualization.snapRevertStore') }}" method="POST" id="formfield">
            {{ csrf_field() }}
            <div class="card card-default">
                <div class="card-header with-border" style="background: #dfe1e4;">
                    <h3 class="card-title">Datos para la ejecución del proceso</h3>
                </div>
                <div class="card-body">
    ...
    <script>
        $(document).ready(function () {
            ...
            send.addEventListener("click", function(){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('virtualization.snapDelete') }}',
                    dataType: 'json',
                    data: {
                        'cod': codigo,
                    },
            ...
            $(document).on("click",".Vlink",function(e){
                selectedItem='<option value="'+$(this).data('id')+','+$(this).data('snap')+','+$(this).data('vcenter')+'" selected>'+$(this).data('name')+'</option>';
