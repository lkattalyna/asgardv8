...
@section('content_header')
    <h1>Formulario de paso a operación virtual host</h1><hr>
@stop
...
@section('content')
    @can('virtualization-run')
        <form action="{{ route('virtualization.checkHostImpleStore') }}" method="POST" id="formfield">
            {{ csrf_field() }}
            <div class="card card-default">
                <div class="card-header with-border" style="background: #dfe1e4;">
                    <h3 class="card-title">Datos para ejecución del check</h3>
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-hover">
                        <tr>
                            <td>
                                <label for="vHost">{{ __('Virtual Host') }}</label>
                            </td>
                            <td>
                                <select id="vHost" name="vHost[]" size="1" multiple class="form-control input-md" style="width: 100%" required></select>
                            </td>
                        </tr>
                    </table>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            send.addEventListener("click", function(){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('virtualization.checkHost') }}',
                    dataType: 'json',
                    data: {
                        'cod': codigo,
                    },
