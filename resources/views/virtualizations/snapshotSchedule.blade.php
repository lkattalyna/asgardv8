...
@section('content_header')
    <h1>Formulario de programación de snapshots</h1><hr>
@stop
...
@section('content')
    @can('virtualization-run')
        <form action="{{ route('virtualization.snapshotScheduleStore') }}" method="POST" id="formfield">
            {{ csrf_field() }}
            <div class="card card-default">
                <div class="card-header with-border" style="background: #dfe1e4;">
                    <h3 class="card-title">Datos para la toma del snapshot</h3>
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-hover">
                        ...
                        <tr>
                            <td>
                                <label for="description">{{ __('Descripción del snapshot') }}</label>
                            </td>
                            <td>
                                <input type="text" name="description" id="description" class="form-control input-md" value="{{ old('description') }}" maxlength="100"
                                        pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-].{3,}" placeholder="Descripción del snapshot | máximo 100 caracteres"  required>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <label for="case">{{ __('Caso relacionado en ServiceManager') }}</label>
                            </td>
                            <td>
                                <input type="text" name="case" id="case" class="form-control input-md" value="{{ old('case') }}" maxlength="20"
                                        pattern="^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-].{2,}" placeholder="# de caso"  required>
                            </td>
                        </tr>
                        @can('virtualization-admin')
                        <tr>
                            <td>
                                <label for="memory">{{ __('Con memoria') }}</label>
                            </td>
                            <td>
                                <select id="memory" name="memory" class="form-control input-md" required>
                        ...
                        <tr>
                            <td>
                                <label for="sDate">{{ __('Fechas de la programación') }}</label>
                            </td>
                            <td>
                                <div class="input-group">
                                    ...
                                    <input type="text" name="sDate" id="sDate" class="form-control input-md" value="{{ old('sDate') }}"
                        ...
                        <tr>
                            <td>
                                <label for="sTime">{{ __('Hora de la programación') }}</label>
                            </td>
                            <td>
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                    ...
                                    <input type="time" name="sTime" id="sTime" class="form-control input-md" value="{{ old('sTime') }}"
