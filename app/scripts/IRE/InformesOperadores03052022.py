    ...
    def DataDocument(self, DataMachine, CPU, MEM, PING):
        ...
        if os.path.isfile(PWD + '/Charts/' + machine + '_CPU.png'):
            ...
            for MPMCPU in CPU:
                ...
                # print("[HEADING]",('2.' + str(ValueNumberFirst) + '.' + str(ValueNumberSecond + 2) + '.\tConsumo CPU\n'))
                ...
                #print("[DATA DOCUMENT] llamando",PWD + '/Charts/' + machine + '_CPU.png')
        ...
        if os.path.isfile(PWD + '/Charts/' + machine + '_PING.png'):
            for MPMPING in PING:
                ...
                #print("[PING]",PING)
                #print("[SumPING]",SumPING)
                #print("[PromedioPING]",PromedioPING)
                #print("[LENG]",len(PING))
...
def analysisOfDatas():
    ...
    NDate = 0
    # LEE TODO EL CSV
    #print('[ANALYSYSOFDATA] DATOSARCHIVO',DatosArchivo)
    for DataFile in range(0, len(DatosArchivo)):
        if 'CPU;' in str(DatosArchivo[DataFile]) or '"CPU";' in str(DatosArchivo[DataFile]):
            if 'CONSUMO DE CPU'.lower() not in str(DatosArchivo[DataFile]).lower():
                # print("[MACHINE APPEND] CPU;",str(DatosArchivo[DataFile]).split(';')[0].replace("['", ''))
    ...
    for MultiplesDate in range(0, len(DATES)):
        #print("[MultiplesDate]",MultiplesDate)
    ...
    for DataMachine in Machine:
        ...
        for DataFile in range(0, len(DatosArchivo)):
            if MachineNameCPU in str(DatosArchivo[DataFile]):
                ...
                for DataCPU in MachineCPU:
                    ...
                    #print("[ValueCPU]",str(DataCPU).replace('\\n', '').replace("']", '').replace(',', '.').replace('"',''))
                    if ValueCPU:
                        if ValueCPU[-1] == '.':
                            ValueCPU = ValueCPU[:-1]
            ...
            if MachineNameCPUS in str(DatosArchivo[DataFile]):
                ...
                for DataCPU in MachineCPU:
                    ...
                    if ValueCPU:
                        ...
                        dataCPUDocument.append(0)
                Charts().CPU(DataMachine, CPU)
            #print('[MACHINENAMEMEMM]',MachineNameMEMS)
            #print('[DatosArchivo[DataFile]]',DatosArchivo[DataFile])
            if MachineNameMEMM in str(DatosArchivo[DataFile]):
                if "swap" not in str(DatosArchivo[DataFile]):
                    ...
                    for DataMEM in MachineMEM:
                        ...
                        #print("[ValueMEM]",str(DataMEM).replace('.', '').replace('\\n', '').replace("']", '').replace(',', '.').replace('"',''))
                        if ValueMEM:
                            if ValueMEM[-1] == '.':
                                ValueMEM = ValueMEM[:-1]
            ...
            if MachineNameMEMS in str(DatosArchivo[DataFile]):
                #print("[MachineNameMEMMS]",MachineNameMEMMS)
        ...
        #print('[DATA MACHINE MEM]',dataMEMDocument)
